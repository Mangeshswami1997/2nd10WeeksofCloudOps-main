provider "aws" {
 ...
 #Creating IAM role for EKS
  ...
  resource "aws_iam_policy" "autoscaler" {
    name = "mangesh-eks-autoscaler-policy1"
    policy = jsonencode({
      "Version": "2012-10-17",
      "Statement": [
        {
  ...
  resource "aws_iam_instance_profile" "worker" {
    ...
    name       = "mangesh-eks-worker-new-profile1"
    role       = aws_iam_role.worker.name
  }
 ...
 data "aws_vpc" "main" {
  tags = {
    Name = "Jumphost-vpc-bastion"  # Specify the name of your existing VPC
  }
}
